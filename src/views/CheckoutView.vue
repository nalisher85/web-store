<template>
  <div class="mx-auto max-w-xl min-h-[100dvh] flex flex-col bg-white">
    ...
    <div ref="scrollEl" class="flex-1 overflow-y-auto px-4"
      :style="{ paddingBottom: `calc(${kb}px + env(safe-area-inset-bottom) + 24px)` }">
      <form @submit.prevent="handleSubmit" class="space-y-4" novalidate>
        <div>
          <label class="block font-semibold mb-1">Имя</label>
          <input v-model.trim="name" type="text" :aria-invalid="attempted && !validName"
            class="w-full border rounded px-3 py-2 field" :class="attempted && !validName ? 'border-red-500' : ''"
            autocomplete="name" @focus="ensureVisible" />
        </div>

        <div>
          <label class="block font-semibold mb-1">Телефон</label>
          <input v-model.trim="phone" type="tel" inputmode="tel" :aria-invalid="attempted && !validPhone"
            class="w-full border rounded px-3 py-2 field" :class="attempted && !validPhone ? 'border-red-500' : ''"
        ...
        <div>
          ...
          <input v-model.trim="address" type="text" :aria-invalid="attempted && !validAddress"
            class="w-full border rounded px-3 py-2 field" :class="attempted && !validAddress ? 'border-red-500' : ''"
        ...
        <div>
          ...
          <input v-model.trim="addressDescr" type="text" placeholder="Подъезд, этаж, ориентир и т.п."
            class="w-full border rounded px-3 py-2 field" @focus="ensureVisible" />
        </div>

        <div>
          <label class="block font-semibold mb-1">Дополнительная информация</label>
          <textarea v-model.trim="extraInfo" rows="3" class="w-full border rounded px-3 py-2 field"
        ...
        <button class="w-full h-12 rounded-lg bg-gray-900 text-white font-semibold
         disabled:opacity-60 disabled:cursor-not-allowed" :disabled="loading || !formValid" type="submit">
